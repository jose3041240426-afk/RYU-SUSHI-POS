...
const STATUS_CONFIG = {
  mucho: { color: '#000', minValue: 2 },
  estable: { color: '#000', exactValue: 1 },
  poco: { color: 'red', maxValue: 0.99 },
...
const InventoryTab = ({ isDarkMode, storageKey, tabStyles }) => {
  ...
  const renderItem = ({ item }) => {
    let statusColor = STATUS_CONFIG[item.status]?.color || 'gray';
    if (statusColor === '#000' && isDarkMode) statusColor = '#fff';
    const borderColor =
      statusColor === 'gray'
